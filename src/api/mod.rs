...
impl<'a> ApiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "attach" => self.plan_attach(current_state, desired_input).await,
            "podtemplate" => self.plan_podtemplate(current_state, desired_input).await,
            "eviction" => self.plan_eviction(current_state, desired_input).await,
            "exec" => self.plan_exec(current_state, desired_input).await,
            "ephemeralcontainer" => {
                self.plan_ephemeralcontainer(current_state, desired_input)
                    .await
            }
            "service" => self.plan_service(current_state, desired_input).await,
            "proxy" => self.plan_proxy(current_state, desired_input).await,
            "node" => self.plan_node(current_state, desired_input).await,
            "log" => self.plan_log(current_state, desired_input).await,
            "serviceaccount" => self.plan_serviceaccount(current_state, desired_input).await,
            "pod" => self.plan_pod(current_state, desired_input).await,
            "binding" => self.plan_binding(current_state, desired_input).await,
            "persistentvolume" => {
                self.plan_persistentvolume(current_state, desired_input)
                    .await
            }
            "limitrange" => self.plan_limitrange(current_state, desired_input).await,
            "configmap" => self.plan_configmap(current_state, desired_input).await,
            "event" => self.plan_event(current_state, desired_input).await,
            "finalize" => self.plan_finalize(current_state, desired_input).await,
            "persistentvolumeclaim" => {
                self.plan_persistentvolumeclaim(current_state, desired_input)
                    .await
            }
            "secret" => self.plan_secret(current_state, desired_input).await,
            "portforward" => self.plan_portforward(current_state, desired_input).await,
            "componentstatuse" => {
                self.plan_componentstatuse(current_state, desired_input)
                    .await
            }
            "scale" => self.plan_scale(current_state, desired_input).await,
            "resourcequota" => self.plan_resourcequota(current_state, desired_input).await,
            "namespace" => self.plan_namespace(current_state, desired_input).await,
            "replicationcontroller" => {
                self.plan_replicationcontroller(current_state, desired_input)
                    .await
            }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "attach" => self.create_attach(input).await,
            "podtemplate" => self.create_podtemplate(input).await,
            "eviction" => self.create_eviction(input).await,
            "exec" => self.create_exec(input).await,
            "ephemeralcontainer" => self.create_ephemeralcontainer(input).await,
            "service" => self.create_service(input).await,
            "proxy" => self.create_proxy(input).await,
            "node" => self.create_node(input).await,
            "log" => self.create_log(input).await,
            "serviceaccount" => self.create_serviceaccount(input).await,
            "pod" => self.create_pod(input).await,
            "binding" => self.create_binding(input).await,
            "persistentvolume" => self.create_persistentvolume(input).await,
            "limitrange" => self.create_limitrange(input).await,
            "configmap" => self.create_configmap(input).await,
            "event" => self.create_event(input).await,
            "finalize" => self.create_finalize(input).await,
            "persistentvolumeclaim" => self.create_persistentvolumeclaim(input).await,
            "secret" => self.create_secret(input).await,
            "portforward" => self.create_portforward(input).await,
            "componentstatuse" => self.create_componentstatuse(input).await,
            "scale" => self.create_scale(input).await,
            "resourcequota" => self.create_resourcequota(input).await,
            "namespace" => self.create_namespace(input).await,
            "replicationcontroller" => self.create_replicationcontroller(input).await,
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "attach" => self.read_attach(id).await,
            "podtemplate" => self.read_podtemplate(id).await,
            "eviction" => self.read_eviction(id).await,
            "exec" => self.read_exec(id).await,
            "ephemeralcontainer" => self.read_ephemeralcontainer(id).await,
            "service" => self.read_service(id).await,
            "proxy" => self.read_proxy(id).await,
            "node" => self.read_node(id).await,
            "log" => self.read_log(id).await,
            "serviceaccount" => self.read_serviceaccount(id).await,
            "pod" => self.read_pod(id).await,
            "binding" => self.read_binding(id).await,
            "persistentvolume" => self.read_persistentvolume(id).await,
            "limitrange" => self.read_limitrange(id).await,
            "configmap" => self.read_configmap(id).await,
            "event" => self.read_event(id).await,
            "finalize" => self.read_finalize(id).await,
            "persistentvolumeclaim" => self.read_persistentvolumeclaim(id).await,
            "secret" => self.read_secret(id).await,
            "portforward" => self.read_portforward(id).await,
            "componentstatuse" => self.read_componentstatuse(id).await,
            "scale" => self.read_scale(id).await,
            "resourcequota" => self.read_resourcequota(id).await,
            "namespace" => self.read_namespace(id).await,
            "replicationcontroller" => self.read_replicationcontroller(id).await,
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "attach" => self.update_attach(id, input).await,
            "podtemplate" => self.update_podtemplate(id, input).await,
            "eviction" => self.update_eviction(id, input).await,
            "exec" => self.update_exec(id, input).await,
            "ephemeralcontainer" => self.update_ephemeralcontainer(id, input).await,
            "service" => self.update_service(id, input).await,
            "proxy" => self.update_proxy(id, input).await,
            "node" => self.update_node(id, input).await,
            "log" => self.update_log(id, input).await,
            "serviceaccount" => self.update_serviceaccount(id, input).await,
            "pod" => self.update_pod(id, input).await,
            "binding" => self.update_binding(id, input).await,
            "persistentvolume" => self.update_persistentvolume(id, input).await,
            "limitrange" => self.update_limitrange(id, input).await,
            "configmap" => self.update_configmap(id, input).await,
            "event" => self.update_event(id, input).await,
            "finalize" => self.update_finalize(id, input).await,
            "persistentvolumeclaim" => self.update_persistentvolumeclaim(id, input).await,
            "secret" => self.update_secret(id, input).await,
            "portforward" => self.update_portforward(id, input).await,
            "componentstatuse" => self.update_componentstatuse(id, input).await,
            "scale" => self.update_scale(id, input).await,
            "resourcequota" => self.update_resourcequota(id, input).await,
            "namespace" => self.update_namespace(id, input).await,
            "replicationcontroller" => self.update_replicationcontroller(id, input).await,
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "attach" => self.delete_attach(id).await,
            "podtemplate" => self.delete_podtemplate(id).await,
            "eviction" => self.delete_eviction(id).await,
            "exec" => self.delete_exec(id).await,
            "ephemeralcontainer" => self.delete_ephemeralcontainer(id).await,
            "service" => self.delete_service(id).await,
            "proxy" => self.delete_proxy(id).await,
            "node" => self.delete_node(id).await,
            "log" => self.delete_log(id).await,
            "serviceaccount" => self.delete_serviceaccount(id).await,
            "pod" => self.delete_pod(id).await,
            "binding" => self.delete_binding(id).await,
            "persistentvolume" => self.delete_persistentvolume(id).await,
            "limitrange" => self.delete_limitrange(id).await,
            "configmap" => self.delete_configmap(id).await,
            "event" => self.delete_event(id).await,
            "finalize" => self.delete_finalize(id).await,
            "persistentvolumeclaim" => self.delete_persistentvolumeclaim(id).await,
            "secret" => self.delete_secret(id).await,
            "portforward" => self.delete_portforward(id).await,
            "componentstatuse" => self.delete_componentstatuse(id).await,
            "scale" => self.delete_scale(id).await,
            "resourcequota" => self.delete_resourcequota(id).await,
            "namespace" => self.delete_namespace(id).await,
            "replicationcontroller" => self.delete_replicationcontroller(id).await,
    ...
    async fn create_attach(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a attach resource
    async fn read_attach(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a attach resource
    async fn update_attach(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a attach resource
    async fn delete_attach(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_podtemplate(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a podtemplate resource
    async fn read_podtemplate(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a podtemplate resource
    async fn update_podtemplate(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a podtemplate resource
    async fn delete_podtemplate(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_eviction(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a eviction resource
    async fn read_eviction(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a eviction resource
    async fn update_eviction(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a eviction resource
    async fn delete_eviction(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_exec(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a exec resource
    async fn read_exec(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a exec resource
    async fn update_exec(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a exec resource
    async fn delete_exec(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_ephemeralcontainer(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a ephemeralcontainer resource
    async fn read_ephemeralcontainer(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_ephemeralcontainer(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_proxy(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a proxy resource
    async fn read_proxy(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a proxy resource
    async fn update_proxy(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn create_serviceaccount(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a serviceaccount resource
    async fn read_serviceaccount(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_serviceaccount(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_pod(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a pod resource
    async fn read_pod(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a pod resource
    async fn update_pod(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn create_binding(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a binding resource
    async fn read_binding(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a binding resource
    async fn update_binding(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn create_persistentvolume(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a persistentvolume resource
    async fn read_persistentvolume(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_persistentvolume(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_limitrange(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a limitrange resource
    async fn read_limitrange(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a limitrange resource
    async fn update_limitrange(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a limitrange resource
    async fn delete_limitrange(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_configmap(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a configmap resource
    async fn read_configmap(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a configmap resource
    async fn update_configmap(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a configmap resource
    async fn delete_configmap(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_finalize(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a finalize resource
    async fn read_finalize(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a finalize resource
    async fn update_finalize(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a finalize resource
    async fn delete_finalize(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_persistentvolumeclaim(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a persistentvolumeclaim resource
    async fn read_persistentvolumeclaim(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_persistentvolumeclaim(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_portforward(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a portforward resource
    async fn read_portforward(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a portforward resource
    async fn update_portforward(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a portforward resource
    async fn delete_portforward(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_componentstatuse(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a componentstatuse resource
    async fn read_componentstatuse(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_componentstatuse(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_scale(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a scale resource
    async fn read_scale(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a scale resource
    async fn update_scale(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a scale resource
    async fn delete_scale(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_resourcequota(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a resourcequota resource
    async fn read_resourcequota(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a resourcequota resource
    async fn update_resourcequota(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a resourcequota resource
    async fn delete_resourcequota(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
    ...
    async fn create_replicationcontroller(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a replicationcontroller resource
    async fn read_replicationcontroller(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Kubernetes SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_replicationcontroller(&self, id: &str) -> Result<()> {
        // TODO: Implement Kubernetes SDK calls
        Ok(())
    }
